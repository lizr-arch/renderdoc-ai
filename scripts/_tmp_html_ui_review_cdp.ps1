...
$jsClick = @'
(async()=>{const stepLog=[];const log=(s)=>stepLog.push(s);try{log('doc_ready:'+document.readyState);log('doc_url:'+document.location.href);}catch(e){log('doc_state_error');}let shaderCount=0;let textureCount=0;let eventCount=0;let contentOk=false;const tryFetch=async(rel)=>{try{const r=await fetch(rel,{cache:'no-store'});if(!r.ok){log('fetch_fail:'+rel+':'+r.status);return null;}return await r.json();}catch(e){log('fetch_error:'+rel);return null;}};try{if(typeof analysisData!=='undefined'&&Array.isArray(analysisData)){analysisData.forEach(fd=>{shaderCount+=(fd.shaders||[]).length;textureCount+=(fd.textures||[]).length;});log('mode:analysisData');log('shader_count:'+shaderCount);log('texture_count:'+textureCount);contentOk=shaderCount>0&&textureCount>0;}else{log('analysisData_missing');log('mode:external');const eventsData=await tryFetch('./events_data.json');if(Array.isArray(eventsData)){eventCount=eventsData.length;}else if(eventsData&&Array.isArray(eventsData.events)){eventCount=eventsData.events.length;}const texturesData=await tryFetch('./textures_data.json');if(Array.isArray(texturesData)){textureCount=texturesData.length;}const shadersData=await tryFetch('./shaders_data.json');if(Array.isArray(shadersData)){shaderCount=shadersData.length;}log('event_count:'+eventCount);log('shader_count:'+shaderCount);log('texture_count:'+textureCount);contentOk=eventCount>0||(shaderCount>0&&textureCount>0);}}catch(e){log('analysisData_error');}const styleId='cdp-style';if(!document.getElementById(styleId)){const s=document.createElement('style');s.id=styleId;s.textContent='[data-cdp-highlight="1"]{outline:3px solid #ff3b30 !important;background:rgba(255,59,48,0.18) !important;}';document.head.appendChild(s);log('inject-style');}const tryClick=(sel)=>{const el=document.querySelector(sel);if(el){el.click();log('clicked:'+sel);return true;}log('missing:'+sel);return false;};if(typeof showEventBrowser==='function'){try{showEventBrowser();log('showEventBrowser()');}catch(e){log('showEventBrowser_error');}}else{tryClick('#eventBrowserBtn');}if(typeof renderEventTree==='function'){try{renderEventTree();log('renderEventTree()');}catch(e){log('renderEventTree_error');}}const sleep=(ms)=>new Promise(r=>setTimeout(r,ms));const maxTries=20;let found=null;for(let i=0;i<maxTries;i++){found=document.querySelector('.event-node')||document.querySelector('.event-tree-list .event-node');if(found){log('event-node-ready');break;}await sleep(250);}const selectors=[{n:'event-node',s:'.event-node'},{n:'event-node-eid',s:'.event-node .event-eid'},{n:'event-tree-list',s:'.event-tree-list .event-node'},{n:'data-event-id',s:'[data-event-id]'},{n:'data-eid',s:'[data-eid]'},{n:'data-eventid',s:'[data-eventid]'},{n:'data-action-id',s:'[data-action-id]'},{n:'event-row',s:'.event-row'},{n:'event-item',s:'.event-item'},{n:'eventRow',s:'.eventRow'},{n:'event-entry',s:'.event-entry'},{n:'action-row',s:'.action-row'},{n:'action-item',s:'.action-item'},{n:'event-list-item',s:'#eventBrowser li, #eventList li'}];function mark(el){const target=el.closest('.event-node')||el;target.scrollIntoView({block:'center'});target.click();target.setAttribute('data-cdp-highlight','1');}for(const q of selectors){const el=document.querySelector(q.s);if(el){mark(el);return {found:true,strategy:q.n,text:(el.textContent||'').trim().slice(0,80),event_count:eventCount,shader_count:shaderCount,texture_count:textureCount,content_ok:contentOk,step_log:stepLog};}}const textRe=/(EID\\s*\\d+|Draw|Dispatch|Present|Clear|Blit|Copy|Resolve|Barrier|Bind)/i;const containers=Array.from(document.querySelectorAll('[id*="event" i],[class*="event" i],[id*="action" i],[class*="action" i],[id*="call" i],[class*="call" i]')).slice(0,20);for(const c of containers){const nodes=c.querySelectorAll('div,li,span,tr');let scanned=0;for(const el of nodes){scanned++;if(scanned>500)break;const t=(el.textContent||'').trim();if(t.length>0 && t.length<120 && textRe.test(t)){mark(el);return {found:true,strategy:'container-text',text:t.slice(0,80),event_count:eventCount,shader_count:shaderCount,texture_count:textureCount,content_ok:contentOk,step_log:stepLog};}}}return {found:false,strategy:'fallback',text:'',event_count:eventCount,shader_count:shaderCount,texture_count:textureCount,content_ok:contentOk,step_log:stepLog};})()
'@
