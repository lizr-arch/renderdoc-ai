...
class RdcXmlParser:
    ...
    Supports both D3D11 and Vulkan captures.
    """
    ...
    # Combined draw calls for quick lookup
    DRAW_CALLS = D3D11_DRAW_CALLS | VK_DRAW_CALLS
    
    # D3D11 Dispatch calls
    D3D11_DISPATCH_CALLS = {
        "ID3D11DeviceContext::Dispatch",
        "ID3D11DeviceContext::DispatchIndirect",
    }
    
    # Vulkan Dispatch calls
    VK_DISPATCH_CALLS = {
        "vkCmdDispatch",
        "vkCmdDispatchIndirect",
    }
    
    DISPATCH_CALLS = D3D11_DISPATCH_CALLS | VK_DISPATCH_CALLS
    ...
    def _parse_draw_call(self, chunk: ET.Element, event_id: int) -> D3D11DrawCall:
        """Parse a draw call chunk (D3D11 or Vulkan)."""
        name = chunk.get("name", "")
        ...
        for child in chunk:
            ...
            # Vulkan parameters (lowercase names)
                ...
                draw.instance_count = int(child.text or 1)
            elif child_name == "firstIndex":
                draw.start_index = int(child.text or 0)
            elif child_name == "vertexOffset":
                draw.base_vertex = int(child.text or 0)
            elif child_name == "firstInstance":
                draw.start_instance = int(child.text or 0)
    ...
    def _parse_texture2d(self, chunk: ET.Element) -> Optional[D3D11Resource]:
        ...
        for child in chunk:
            ...
            # D3D11: <ResourceId name="pTexture" ...>281</ResourceId>
            if child_name == "pTexture" and (child.tag == "ResourceId" or child.tag == "uint"):
                resource_id = int(child.text or 0)
    ...
    def _extract_vk_marker_text(self, chunk: ET.Element) -> str:
        """Extract the marker text from a Vulkan debug marker chunk."""
        # Look for pLabelInfo/pMarkerInfo struct
        for child in chunk:
            child_name = child.get("name", "")
            if child_name in ("pLabelInfo", "pMarkerInfo"):
                # Find pLabelName inside the struct
                for struct_child in child:
                    if struct_child.get("name") == "pLabelName":
                        return struct_child.text or ""
            elif child_name == "pMarkerName":
                return child.text or ""
        return ""
    
    def _parse_vk_buffer(self, chunk: ET.Element) -> Optional[D3D11Resource]:
        """Parse a vkCreateBuffer chunk.
        
        XML structure:
            <chunk name="vkCreateBuffer">
                <ResourceId name="device">9</ResourceId>
                <struct name="CreateInfo" typename="VkBufferCreateInfo">
                    <uint name="size">1024</uint>
                    <enum name="usage" string="VK_BUFFER_USAGE_*">flags</enum>
                </struct>
                <ResourceId name="Buffer">123</ResourceId>
            </chunk>
        """
        resource_id = 0
        byte_width = 0
        ...
        for child in chunk:
            ...
            if child_name == "Buffer" and child.tag == "ResourceId":
                resource_id = int(child.text or 0)
            elif child_name == "CreateInfo":
                for desc_child in child:
                    desc_name = desc_child.get("name", "")
                    if desc_name == "size":
                        byte_width = int(desc_child.text or 0)
    ...
    def _parse_vk_image(self, chunk: ET.Element) -> Optional[D3D11Resource]:
        """Parse a vkCreateImage chunk.
        
        XML structure:
            <chunk name="vkCreateImage">
                <struct name="CreateInfo" typename="VkImageCreateInfo">
                    <enum name="imageType" string="VK_IMAGE_TYPE_2D">1</enum>
                    <enum name="format" string="VK_FORMAT_R8G8B8A8_UNORM">37</enum>
                    <struct name="extent" typename="VkExtent3D">
                        <uint name="width">1920</uint>
                        <uint name="height">1080</uint>
                        <uint name="depth">1</uint>
                    </struct>
                    <uint name="mipLevels">1</uint>
                    <uint name="arrayLayers">1</uint>
                    <enum name="usage" string="VK_IMAGE_USAGE_*">flags</enum>
                </struct>
                <ResourceId name="Image">133</ResourceId>
            </chunk>
        """
        resource_id = 0
        width = 0
        ...
        for child in chunk:
            ...
            if child_name == "Image" and child.tag == "ResourceId":
                resource_id = int(child.text or 0)
            elif child_name == "CreateInfo":
                for desc_child in child:
                    desc_name = desc_child.get("name", "")
                    
                    if desc_name == "imageType":
                        type_str = desc_child.get("string", "")
                        if "3D" in type_str:
                            image_type = "Texture3D"
                        elif "1D" in type_str:
                            image_type = "Texture1D"
                        else:
                            image_type = "Texture2D"
                    elif desc_name == "format":
                        tex_format = desc_child.get("string", "")
                    elif desc_name == "extent":
                        for ext_child in desc_child:
                            ext_name = ext_child.get("name", "")
                            if ext_name == "width":
                                width = int(ext_child.text or 0)
                            elif ext_name == "height":
                                height = int(ext_child.text or 0)
                            elif ext_name == "depth":
                                depth = int(ext_child.text or 1)
                    elif desc_name == "mipLevels":
                        mip_levels = int(desc_child.text or 1)
                    elif desc_name == "arrayLayers":
                        array_size = int(desc_child.text or 1)
                    elif desc_name == "usage":
                        usage = desc_child.get("string", "")
