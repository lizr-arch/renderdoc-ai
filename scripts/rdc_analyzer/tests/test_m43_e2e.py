...
def _extract_embedded_json(html: str, candidates):
    """Extract embedded JSON assigned to one of the candidate JS variables."""
    ...
    raise AssertionError(f"未找到嵌入 JSON 变量: {candidates}")
...
def test_data_structure_generation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shader_data = _extract_embedded_json(html_content, ["embeddedData", "shaderData"])
        ...
        assert mali["uniformRegisters"] == 16, "uniformRegisters 错误"
        assert mali["stackSpilling"] is False, "stackSpilling 错误"
        assert mali["hasLateZS"] is False, "hasLateZS 错误"
        ...
        assert dm["pixelCoverage"] == 1.0, (
            f"pixelCoverage 错误: {dm['pixelCoverage']} (期望 1.0，因为有 bloom)"
        )
...
def test_health_score_algorithm():
    ...
    spec = importlib.util.spec_from_file_location(
        "shader_perf_analyzer", str(Path(__file__).parent.parent / "analyzers" / "shader_perf_analyzer.py")
    )
    ...
    assert 83 <= calculate_register_score(32, False) <= 87, (
        f"32 regs 应约为 85 分: {calculate_register_score(32, False)}"
    )
...
def test_coverage_heuristic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pass_name, expected_coverage in test_cases:
            ...
            mock_usage = {"s1": [{"event_id": 1, "draw_name": pass_name, "slot": 0}]}

            gen.set_shaders(mock_shaders, mali_data={}, usage_map=mock_usage)
            html = gen.generate_shaders()

            data = _extract_embedded_json(html, ["embeddedData", "shaderData"])
