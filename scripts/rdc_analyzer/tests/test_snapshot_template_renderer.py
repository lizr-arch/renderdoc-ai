...
def test_snapshot_renderer_outputs_minimum_pages(tmp_path: Path):
    snapshot = {
        ...
        "availability": {"status": "partial", "mcp_hint": "Use MCP query"},
    }
    ...
    for page in ["index.html", "events.html", "textures.html", "shaders.html", "recommendations.html"]:
    ...
    assert manifest["pages"] == ["index", "events", "textures", "shaders", "recommendations"]
