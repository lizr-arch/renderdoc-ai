...
测试 XML 解析数据到 AnalysisContext 的转换逻辑。
...
# 直接从 core 子模块导入，避免触发 __init__.py 的问题导入
...
class TestXMLToContextBridge:
    """测试 XMLToContextBridge 类"""
    ...
    def test_convert_basic_metadata(self):
        """测试基础元数据转换"""
        xml_data = {
            'apiType': 'D3D11',
            'events': [],
    ...
    def test_convert_draw_calls_d3d11(self):
    ...
    def test_convert_draw_calls_vulkan(self):
    ...
    def test_convert_textures(self):
        """测试纹理转换"""
        xml_data = {
            'apiType': 'D3D11',
            'events': [],
    ...
    def test_is_draw_call_detection(self):
        """测试 Draw Call 类型检测"""
    ...
    def test_is_dispatch_detection(self):
        """测试 Dispatch 类型检测"""
        bridge = XMLToContextBridge
        
        assert bridge._is_dispatch('Dispatch') == True
        assert bridge._is_dispatch('vkCmdDispatch') == True
        assert bridge._is_dispatch('glDispatchCompute') == True
        
        assert bridge._is_dispatch('DrawIndexed') == False
    
    def test_texture_format_categorization(self):
        """测试纹理格式分类"""
        bridge = XMLToContextBridge
    ...
    def test_safe_int_conversion(self):
        ...
        assert bridge._safe_int(42) == 42
        assert bridge._safe_int('100') == 100
        assert bridge._safe_int('invalid') == 0
        assert bridge._safe_int(None) == 0
        assert bridge._safe_int(3.14) == 0  # float 不转换
    
    def test_parsed_data_preservation(self):
        """测试原始数据保留"""
        xml_data = {
            'apiType': 'Vulkan',
            'events': [
...
class TestXMLToContextBridgeEdgeCases:
    """边界情况测试"""
    
    def test_missing_pipeline_state(self):
    ...
    def test_string_usage_field(self):
        """测试 usage 字段为字符串的情况"""
        xml_data = {
            'buffers': [
                {
                    'resourceId': '0x1',
                    'usage': 'ConstantBuffer',  # 字符串而非列表
                }
            ]
        }
        ...
        assert context.buffers[0].is_constant_buffer == True
    
    def test_instance_count_default(self):
        """测试 instance count 默认值"""
        xml_data = {
            'events': [
                {
                    'eventId': 1,
        ...
        assert context.draw_calls[0].instance_count == 1
