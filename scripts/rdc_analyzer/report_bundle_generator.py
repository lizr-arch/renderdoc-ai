...
class ReportBundleGenerator:
    ...
    def generate_all(self) -> Dict[str, str]:
        ...
        # 7. 生成 RT 预览服务启动脚本
        startup_scripts = self._generate_rt_server_scripts()
    ...
    def _generate_rt_server_scripts(self) -> Dict[str, str]:
        """
        生成 RT 预览服务的启动脚本
...
'''
        ...
        bat_path = self.output_dir / "start_rt_server.bat"
...
'''
        ...
        sh_path = self.output_dir / "start_rt_server.sh"
...
2. 或使用 Python Headless（推荐）：
   ```
   py -3 rt_preview_server.py --rdc "path\\to\\your_capture.rdc" --port {self.rt_server_port}
   ```
3. 如需加载真实 RDC 文件：
   ```
...
- **端口**: {self.rt_server_port}
- **API 端点**: `http://localhost:{self.rt_server_port}/api/rt/{{eid}}`
- **纹理端点**: `http://localhost:{self.rt_server_port}/api/texture/{{id}}`
...
'''
        ...
        readme_path = self.output_dir / "RT_SERVER_README.md"
