...
### 5.8 字段级清单（Resources / Actions）

**ResourceDescription（资源元数据）**  
- 字段：`resourceId, type, autogeneratedName, name, initialisationChunks, derivedResources, parentResources, annotations`  
- Python 入口：`ReplayController.GetResources()`  
- 覆盖现状：A/C 缺失；B 可通过 Replay 获取  
