...
### 1.1.2 Coverage（数据丰富度覆盖与缺口）

- WHAT：为 `events[]` 与 `textures[]` 增加 `coverage` 字段，描述 RenderDoc 标准字段的覆盖情况。
  - `coverage.present`：已覆盖字段（完全匹配）
  - `coverage.partial`：部分覆盖字段（映射来源 + 原因）
  - `coverage.missing`：缺失字段（必须给出原因）
  - `coverage.standard`：标准来源（RenderDoc）
