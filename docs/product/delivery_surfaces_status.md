...
> conflict_points:
> - A 线已明确收口为 `A-runtime-surface candidate`，但更大方法集的 repo-local handler/source 仍未确认，需保留 `A-contract-followup`
...
| MCP 查询 | `run_query.py` / bridge client / `snapshot_consumer.py` 消费 `mcp-query.v1` envelope | A candidate 已并入 merge gate 分支；merged live gate 中 `get_capture_status` / `get_frame_summary` / `snapshot_consume --execute` 均通过 | 更大方法集的 repo-local handler/source 仍未定位 |
...
已经在 repo-local GUI handler/source 里被完整验证
...
- `A-contract-followup` 继续保留：
  - repo-local handler/source 未闭口前，不宣称更大方法集已落地
...
- A 线更大方法集仍保留为 `A-contract-followup`
...
- A：当前周期已满足 `runtime-surface candidate` 收口；后续只剩 `A-contract-followup`
...
- 当前周期范围已明确收敛为 `A-runtime-surface candidate`
- candidate SHA：`8e1a159ce7c9e58839e9db21d5ba09ae84a03956`
- 剩余工作仅为 `A-contract-followup`：
  - 继续追 GUI handler/source
  - 评估更大方法集何时恢复到 repo-local 可验证状态
...
- 剩余工作：本轮 PR/GitHub/main 合流已完成；后续只应单开维护/回归或 `A-contract-followup`
...
- 当前 merge gate 中已验证并用于 smoke/consumer 的 runtime-surface 主要覆盖：
  - `get_capture_status`
  - `get_frame_summary`
  - `get_pipeline_state`
  - `get_texture_data`
- 因此 A 线仍只能宣称 `runtime-surface candidate`，不能宣称完整 `mcp-query.v1` repo-local handler/source 闭口。

命令证据：

- `Select-String -Path D:\Code\git\renderdoc\docs\product\mcp_query_contract_v1.md -Pattern '^##|method|handler|source|get_capture_status|get_frame_summary|get_pipeline_state|get_draw|get_texture|get_buffer|request\.json|response\.json'`
- `Get-ChildItem -LiteralPath D:\Code\git\renderdoc-merge-gate-20260424\tools\mcp -Recurse -File -Include *.py | Select-String -Pattern 'get_capture_status|get_frame_summary|get_pipeline_state|get_draw|get_texture|get_buffer|request\.json|response\.json|METHODS|method'`
- `Get-ChildItem -LiteralPath D:\Code\git\renderdoc-merge-gate-20260424\scripts\rdc_analyzer\mcp_examples -Recurse -File -Include *.py | Select-String -Pattern 'get_capture_status|get_frame_summary|get_pipeline_state|get_draw|get_texture|get_buffer|request\.json|response\.json|METHODS|method'`

下一步：

- 单开 `A-contract-followup`，从 `renderdoc-ai/main@25fd5be9dc844a59a4b10897c7b4105141dcf127` 追 repo-local handler/source。
- 不在当前 A/B merge-gate 上继续扩大实现面。
...
- 当前两份状态入口已继续承接 P1-P3 follow-up，不新增平行 current-status 文档。
- 三份控制文件仍未纳入 git，后续若要提交，应单独做 docs/control 提交，不混入业务实现。

命令证据：

- `git -C D:\Code\git\renderdoc status --short --branch`
- `git -C D:\Code\git\renderdoc status --short -- docs\product\delivery_surfaces_status.md docs\answers\renderdoc_ai_current_delivery_status.md plans\2026-04-23-213050-Lead-A-Then-B-Gap-Closure.md`
...
- 默认 `gh` 仍被用户目录配置权限阻断：
  - `C:\Users\lizhirui01\AppData\Roaming\GitHub CLI\config.yml: Access is denied`
- 使用仓库内临时 `GH_CONFIG_DIR` 后，`gh --version` 可正常运行：
  - `gh version 2.87.3 (2026-02-23)`
- 临时配置目录没有登录态，`gh auth status --hostname github.com` 返回未登录。
- 因此工具链问题已从“CLI 不可启动”降级为“默认配置文件权限 + 临时配置未认证”。

命令证据：

- `gh --version`
- `icacls "C:\Users\lizhirui01\AppData\Roaming\GitHub CLI\config.yml"`
- `New-Item -ItemType Directory -Force -Path 'D:\Code\git\renderdoc\.tmp-gh-config' | Out-Null; $env:GH_CONFIG_DIR='D:\Code\git\renderdoc\.tmp-gh-config'; gh --version`
- `$env:GH_CONFIG_DIR='D:\Code\git\renderdoc\.tmp-gh-config'; gh auth status --hostname github.com`

下一步：

- 若后续仍需 `gh` 执行 PR 操作，有两条路径：
  - 修复默认配置文件权限后继续使用默认 `gh`。
  - 保留 `GH_CONFIG_DIR=D:\Code\git\renderdoc\.tmp-gh-config`，重新执行 `gh auth login` 建立隔离登录态。
